...
const props = {
    ...
    onChanged: Function as PropType<(config: ConfigSchema) => any>
}
...
export default defineComponent({
    ...
    setup(props) {
        ...
        onMounted(() => {
            watch(() => props.config, () => {
                ...
                nextTick(() => {
                    ...
                    setting.on('changed', changedHandler)
                })
            }, {
                immediate: true,
            })
        ...
        function changedHandler() {
            props.onChanged?.(setting!.toSchema())
        }
            ...
            <div ref="rootDom" class="relative">
                {
                    vFor(this.nodes, (node) => <div
                        ...
                        id={nodeId(node.id)}
                        key={this.$props.config?.id + node.id}
                        style={{
                            left: node.x + 'px',
                            top: node.y + 'px',
                        }}
